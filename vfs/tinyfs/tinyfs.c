...
#include "tinyfs.h"
  ...
  #define LINUX_VERSION_CODE 10000000
#endif
...
static int tinyfs_readdir(struct file *fp, void *dirent, filldir_t filldir) {
    loff_t pos;
    struct file_blk *blk;
    ...
    // blk = (struct file_blk *)fp->f_dentry->d_inode->i_private;
    ...
    for (i = 0; i < blk->dir_children; i++) {
        ...
        fp->f_pos += sizeof(struct dir_entry);
...
static int tinyfs_iterate(struct file *fp, struct dir_context *ctx) {

    struct inode *dir_vi = file_inode(fp);
    ...
    blk = (struct file_blk *)(fp->f_path.dentry->d_inode->i_private);
    if (!S_ISDIR(blk->mode)) {
        return -ENOTDIR;
    }
...
ssize_t tinyfs_read(struct file *fp, char __user *buf, size_t len, loff_t *ppos) {
    struct file_blk *blk;
    char *buffer;
...
ssize_t tinyfs_write(struct file *fp, const char __user *buf, size_t len, loff_t *ppos) {
    struct file_blk *blk;
    char *buffer;

    blk = fp->f_path.dentry->d_inode->i_private;
    buffer = (char *)&blk->data[0];
    buffer += *ppos;
...
static int tinyfs_create(struct inode *dir, struct dentry *dentry, umode_t mode) {
